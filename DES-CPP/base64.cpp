...
size_t Base64::DecodedLength(const char *in, size_t in_length) {
    if (in_length == 0) {
        return 0;
    }
...
size_t Base64::DecodedLength(const std::string &in) {
    if (in.empty()) {
        return 0;
    }
...
size_t Base64::EncodedLength(const std::string &in) {
